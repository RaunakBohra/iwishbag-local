...
export async function calculateCustomsTier(
  ...
  try {
    ...
    console.log(
      `📋 Found ${tiers?.length || 0} customs tiers for ${originCountry} → ${destinationCountry}`,
    );
    ...
    if (appliedTier) {
      console.log(
        `✅ Applied tier: ${appliedTier.rule_name} (${appliedTier.customs_percentage}% customs, ${appliedTier.vat_percentage}% VAT)`,
      );
      return {
    ...
    console.log(
      `⚠️ No matching customs tier found for ${originCountry} → ${destinationCountry}, using fallback`,
    );
    return {
...
function determineAppliedTier(
  ...
  for (const tier of tiers) {
    const priceMatch =
      (!tier.price_min || price >= tier.price_min) && (!tier.price_max || price <= tier.price_max);
