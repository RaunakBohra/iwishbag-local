...
export function usePaymentMonitoring(options: UsePaymentMonitoringOptions = {}) {
   ...
   */
  const monitorPaymentStart = useCallback(
    (params: {
      paymentId: string;
      gateway: string;
      amount: number;
  ...
  const logPaymentEvent = useCallback(
    (event: string, details: Record<string, unknown>, paymentId?: string) => {
  ...
  const logPaymentError = useCallback(
    (event: string, error: Error, details?: Record<string, unknown>, paymentId?: string) => {
      logger.error(LogCategory.PAYMENT_PROCESSING, `Payment error: ${event}`, error, {
        paymentId,
        metadata: {
          event,
          ...details,
   ...
   */
  const monitorGatewayCall = useCallback(
    async <T>(operation: string, apiCall: () => Promise<T>, paymentId?: string): Promise<T> => {
...
export function useWebhookMonitoring(gateway: string) {
  ...
  const trackWebhookProcessing = useCallback(
    async <T>(eventType: string, processor: () => Promise<T>, paymentId?: string): Promise<T> => {
