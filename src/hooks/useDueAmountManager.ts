...
export function useDueAmountManager({
  ...
  const handleOrderValueChange = useCallback(
    async (newTotal: number, quote?: Quote) => {
   ...
   */
  const generatePaymentLinkForDue = useCallback(
    async (dueAmount: number, quote?: Quote) => {
      if (!quote) {
   ...
   */
  const sendPaymentLinkEmailNotification = useCallback(
    async (
  ...
  const generatePaymentLink = useCallback(
    async (amount: number, quote?: Quote) => {
  ...
  useEffect(() => {
    const checkExistingDueAmount = async () => {
      ...
      try {
        ...
        if (quote) {
          setLastKnownTotal(parseFloat(quote.final_total_origincurrency) || 0);
        }
      } catch (error) {
