...
import { priceFormatter, PriceOptions, DualPriceOptions, PriceResult, DualPriceResult } from '@/lib/PriceFormatter';
...
export function usePriceWithCache(options: UsePriceWithCacheOptions): UsePriceWithCacheResult {
  ...
  const userPreferredCurrency = options.userPreferredCurrency || userProfile?.preferred_display_currency;

  const formatPrice = useCallback((amount: number | null | undefined): PriceResult => {
  ...
  }, [options.originCountry, options.destinationCountry, userPreferredCurrency, effectiveExchangeRate, options.showWarnings, exchangeRateWarning]);

  const formatDualPrice = useCallback((amount: number | null | undefined): DualPriceResult | null => {
  ...
  }, [options.originCountry, options.destinationCountry, effectiveExchangeRate, options.showWarnings, exchangeRateWarning]);
