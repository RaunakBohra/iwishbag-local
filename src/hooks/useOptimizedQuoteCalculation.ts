...
export function useOptimizedQuoteCalculation(
  ...
  const calculateQuote = useCallback(async (params: QuoteCalculationParams): Promise<QuoteCalculationResult> => {
  ...
  const calculateQuoteSync = useCallback((params: QuoteCalculationParams) => {
  ...
  const calculateBatch = useCallback(async (
    ...
    try {
      ...
      for (let i = 0; i < calculations.length; i += BATCH_SIZE) {
        ...
        setProgress({ completed: i + batch.length, total: calculations.length });
      }
