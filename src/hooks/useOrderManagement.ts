    ...
    queryFn: async () => {
      ...
      if (statusFilter !== 'all') {
        ...
        if (orderStatusNames.includes(statusFilter)) {
          ...
          console.log(
            `DEBUG: Status '${statusFilter}' is not allowed in orders list, showing all allowed statuses`,
          );
      ...
      if (searchTerm) {
        ...
        query = query.or(
          `order_display_id.ilike.${searchString},product_name.ilike.${searchString},email.ilike.${searchString},display_id.ilike.${searchString}`,
        );
      }
