...
export const useUnifiedQuote = ({
  ...
  const applySuggestion = useCallback(
    async (suggestion: SmartSuggestion): Promise<boolean> => {
      if (!quote) return false;
  ...
  const updateItem = useCallback(
    async (itemId: string, updates: any): Promise<boolean> => {
      if (!quote) return false;
  ...
  const getQuoteMetrics = useCallback(() => {
    ...
    const avgWeightConfidence =
      quote.items.reduce((sum, item) => sum + (item.smart_data?.weight_confidence || 0), 0) /
      quote.items.length;
