...
export function usePrice(options: UsePriceOptions): UsePriceResult {
  ...
  const formatPrice = useCallback(async (amount: number | null | undefined): Promise<PriceResult> => {
    setIsLoading(true);
    setError(null);
  ...
  }, [effectiveOptions.originCountry, effectiveOptions.destinationCountry, effectiveOptions.userPreferredCurrency, effectiveOptions.exchangeRate]);

  const formatDualPrice = useCallback(async (amount: number | null | undefined): Promise<DualPriceResult> => {
...
export function useSimplePrice(options: UseSimplePriceOptions) {
  ...
  const formatAmount = useCallback(async (amount: number | null | undefined): Promise<string> => {
...
export function useAdminPrice(options: { originCountry: string; destinationCountry: string }) {
  ...
  const formatAdminPrice = useCallback(async (amount: number | null | undefined): Promise<DualPriceResult> => {
    setIsLoading(true);
    setError(null);
