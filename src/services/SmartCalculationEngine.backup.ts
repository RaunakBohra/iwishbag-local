...
export class SmartCalculationEngine {
  ...
  private calculateCompleteCostsSyncDirect(params: {
    ...
    // ✅ REMOVED: vatAmount is deprecated - destinationTax is now calculated properly
    
    // ✅ FIXED: Calculate totals using ACTUAL product prices (itemsTotal) - NO DOUBLE COUNTING
    const subtotal =
      itemsTotal +
      shippingCost +
      customsAmount +
      salesTax +
      destinationTax + // This already includes VAT/GST from destination country
      handlingFee +
      insuranceAmount +
      domesticShipping;
    ...
    const updatedQuote: UnifiedQuote = {
      ...
      calculation_data: {
        ...
        breakdown: {
          ...
          taxes: salesTax + destinationTax, // ✅ FIXED: Combined all taxes for consistency
  ...
  private async calculateCompleteCosts(params: {
    ...
    // ✅ NEW: Calculate purchase tax (sales tax at origin) first
    let purchaseTaxAmount = 0;
    let actualItemCost = itemsTotal; // Will be updated with purchase tax
    let paymentGatewayFee = 0; // Declare early to use throughout
    
    // Get route data to check for origin sales tax
    const totalWeight = quote.items.reduce((sum, item) => sum + (item.weight || 0) * (item.quantity || 1), 0);
    ...
    if (routeRates && routeRates.sales_tax > 0) {
      purchaseTaxAmount = (itemsTotal * routeRates.sales_tax) / 100;
      actualItemCost = itemsTotal + purchaseTaxAmount;
      console.log(`[PURCHASE TAX] Applied ${routeRates.sales_tax}% sales tax:`, {
        route: `${quote.origin_country}→${quote.destination_country}`,
        items_total: itemsTotal,
        purchase_tax_rate: routeRates.sales_tax,
        purchase_tax_amount: purchaseTaxAmount,
        actual_item_cost: actualItemCost
      });
    } else {
      console.log(`[PURCHASE TAX] No sales tax for route ${quote.origin_country}→${quote.destination_country}`);
    }
    ...
    // ✅ CRITICAL FIX: Use customsCalculationBase (respects valuation method) for CIF
    ...
    if (shouldUsePerItemTaxes) {
      ...
      for (const item of quote.items) {
        ...
        if (itemTaxMethod === 'hsn' && item.hsn_code) {
          ...
          // Use only the manual rate from tax_options, no fallback
          itemCustomsRate = item.tax_options?.manual?.rate || 0;
        ...
        // Calculate item customs using same valuation method as quote-level
        // Get the item's valuation-adjusted value based on quote's valuation method
        const itemValuationValue = (customsCalculationBase / itemsTotal) * itemValue;
        const itemCustoms = (itemValuationValue * itemCustomsRate) / 100;
        ...
        console.log(`[TAX CALCULATION SUMMARY] Item ${item.id} (${item.name}):`, {
          ...
          valuation_adjusted_value: itemValuationValue,
          rates: {
          ...
          amounts: {
            customs: `$${itemCustoms.toFixed(2)} (${itemValuationValue.toFixed(2)} × ${itemCustomsRate}%)`,
        ...
        itemBreakdowns.push({
          ...
          customs_value: itemValuationValue, // Use valuation-adjusted value
          item_value: itemValue, // Keep original item value for reference
          valuation_method: quote.valuation_method_preference || 'product_value'
        });
      }
      ...
      for (const item of quote.items) {
        if (item.hsn_code) {
          ...
          if (hsnRates) {
            // Calculate item-level taxes using same valuation method as quote-level
            const itemValuationValue = (customsCalculationBase / itemsTotal) * itemValue;
            hsnBasedCustomsTotal += (itemValuationValue * hsnRates.customs) / 100;
    ...
    try {
      ...
      if (vatResult.percentage > 0) {
        ...
        // ✅ CRITICAL FIX: Calculate payment gateway fees BEFORE destination tax
        const preGatewaySubtotal =
          actualItemCost + // Use actual item cost including purchase tax
          selectedShipping.cost_usd +
          insuranceAmount +
          customsAmount +
          handlingFee +
        ...
        // Calculate payment gateway fee on pre-tax subtotal
        paymentGatewayFee = preGatewaySubtotal * 0.029 + 0.3;
        ...
        console.log(`[DESTINATION TAX DEBUG] Calculated destination tax with gateway fees:`, {
          ...
          pre_gateway_subtotal: preGatewaySubtotal,
          payment_gateway_fee: paymentGatewayFee,
        ...
        // Still calculate payment gateway fee even if no destination tax
        const preGatewaySubtotal = actualItemCost + selectedShipping.cost_usd + insuranceAmount + customsAmount + handlingFee + localTaxesAmount;
        paymentGatewayFee = preGatewaySubtotal * 0.029 + 0.3;
        console.log(`[DESTINATION TAX DEBUG] No VAT/GST for ${quote.destination_country}, but calculated gateway fee: $${paymentGatewayFee.toFixed(2)}`);
      }
    } catch (error) {
      ...
      // Fallback gateway fee calculation
      const preGatewaySubtotal = actualItemCost + selectedShipping.cost_usd + insuranceAmount + customsAmount + handlingFee + localTaxesAmount;
      paymentGatewayFee = preGatewaySubtotal * 0.029 + 0.3;
      // Try to calculate rate from existing amount
      if (destinationTaxAmount > 0) {
        destinationTaxRate = (destinationTaxAmount / (preGatewaySubtotal + paymentGatewayFee)) * 100;
      }
    }
    ...
    const updatedCalculationData: CalculationData = {
      ...quote.calculation_data,
      breakdown: {
        items_total: itemsTotal + purchaseTaxAmount, // ✅ FIXED: Include purchase tax in items_total for consistency
        shipping: selectedShipping.cost_usd,
        customs: customsAmount,
        taxes: salesTax + localTaxesAmount + vatAmount + destinationTaxAmount, // ✅ FIXED: Combined all taxes for consistency
        fees: paymentGatewayFee,
        handling: handlingFee,
        insurance: insuranceAmount,
        discount: totalDiscountAmount,
    ...
    const updatedOperationalData: OperationalData = {
      ...
      purchase_tax_amount: purchaseTaxAmount,
      purchase_tax_rate: routeRates?.sales_tax || 0,
      actual_item_cost: actualItemCost,
