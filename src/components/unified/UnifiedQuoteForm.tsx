...
const createValidationSchema = (locale: Locale, isEditMode: boolean) => {
  ...
  return z.object({
    ...
    priority: isEditMode ? z.enum(['low', 'medium', 'high', 'urgent']).optional() : z.never().optional(),
  });
};
...
interface UnifiedQuoteFormProps {
  ...
  // Server validation
  serverValidation?: {
    validateUrl?: (url: string) => Promise<{ valid: boolean; productName?: string; price?: number; error?: string }>;
...
export const UnifiedQuoteForm = memo<UnifiedQuoteFormProps>(({
  ...
  const handleSubmit = useCallback(async (data: FormData) => {
    ...
    try {
      ...
      if (performanceMode === 'detailed') {
        const formComplexity = uploadedFiles.length > 0 ? 'complex' : 
                              enableAdvancedFields ? 'standard' : 'simple';
  ...
  }, [onSubmit, uploadedFiles, performanceMode, startTime, userType, locale, trackConversion, enableAdvancedFields]);
  ...
  // Render form section
  const renderFormSection = (title: string, children: React.ReactNode, icon?: React.ReactNode) => (
    ...
    <Card className={cn(
      ...
      <CardContent>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
                  ...
                  render={({ field }) => (
                    <div className="space-y-1">
                      ...
                      {form.formState.errors.destinationCountry && (
                        <p className="text-sm text-red-600">{form.formState.errors.destinationCountry.message}</p>
                      )}
                    </div>
                  )}
                />
          ...
          {/* Customer Information (for guest users) */}
          {viewMode === 'guest' && renderFormSection(
          ...
          {/* File Upload */}
          {enableFileUpload && renderFormSection(
            t('uploadFiles'),
            <div className="space-y-4">
