...
const fuzzySearch = (searchIndex: ReturnType<typeof createSearchIndex>, searchTerm: string): number[] => {
...
export const UnifiedQuoteList = memo<UnifiedQuoteListProps>(({
  ...
  const processedQuotes = useMemo(() => {
    ...
    if (debouncedSearchTerm.trim()) {
      ...
      if (performanceMode === 'detailed') {
        console.log(`Search took ${performance.now() - searchStartTime}ms for "${debouncedSearchTerm}"`);
      }
    }
    ...
    if (filters.dateRange.start || filters.dateRange.end) {
      result = result.filter(quote => {
        ...
        return (!filters.dateRange.start || quoteDate >= filters.dateRange.start) &&
    ...
    if (filters.priceRange.min !== undefined || filters.priceRange.max !== undefined) {
      result = result.filter(quote => {
        ...
        return (filters.priceRange.min === undefined || price >= filters.priceRange.min) &&
  ...
  const handleSelectAll = useCallback(() => {
    ...
    const newSelection = selectedItems.size === allIds.length ? new Set<string>() : new Set(allIds);
    setSelectedItems(newSelection);
    onSelectionChange?.(Array.from(newSelection));
  }, [processedQuotes, selectedItems.size, onSelectionChange]);
  
  const handleItemAction = useCallback((action: string, quote: UnifiedQuote, optimistic?: boolean) => {
  ...
  const handleSort = useCallback((field: SortOption['field']) => {
          ...
          <div className="flex items-center gap-2">
            {enableSmartCaching && performanceMode === 'detailed' && (
              <Badge variant="outline" className="text-xs">
                Cache: {Math.round(cacheStats.hits / Math.max(cacheStats.hits + cacheStats.misses, 1) * 100)}%
              </Badge>
            )}
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              >
                ...
                {selectedItems.size === 0 ? 'Select All' : 
                 selectedItems.size === processedQuotes.length ? 'Deselect All' : 
          ...
          {enableSorting && (
            <div className="flex items-center gap-2 text-sm">
              ...
              {[
                { field: 'created_at' as const, label: 'Date', icon: <Calendar className="h-3 w-3" /> },
                { field: 'final_total_usd' as const, label: 'Amount', icon: <DollarSign className="h-3 w-3" /> },
                { field: 'status' as const, label: 'Status', icon: <Clock className="h-3 w-3" /> },
                { field: 'customer_name' as const, label: 'Customer', icon: <User className="h-3 w-3" /> }
