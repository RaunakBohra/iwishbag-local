        ...
        <Card>
            ...
            {isLoading ? (
                ...
                {[1, 2, 3].map((i) => (
                    ...
                    <div className="h-6 bg-gray-200 rounded w-1/3 mb-2 dark:bg-gray-700"></div>
                    <div className="h-16 bg-gray-200 rounded w-full dark:bg-gray-700"></div>
                    </div>
                ))}
                </div>
            ) : messages && messages.length > 0 ? (
                <Accordion type="single" collapsible className="w-full" defaultValue={defaultAccordionItem}>
                {Object.entries(customerConversations)
                    .sort(([keyA], [keyB]) => {
                    if (keyA === 'general') return -1;
                    if (keyB === 'general') return 1;
                    const lastMessageA = customerConversations[keyA]?.[customerConversations[keyA].length - 1];
                    const lastMessageB = customerConversations[keyB]?.[customerConversations[keyB].length - 1];
                    ...
                    const title = key === 'general' 
                        ? 'General Support' 
                        : `Quote #${key.substring(0, 8)}`;
