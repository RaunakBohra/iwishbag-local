          ...
          <div className="flex items-center justify-between">
            ...
            {dueInfo.changeType !== 'none' && (
              <Badge variant={dueInfo.changeType === 'increase' ? 'destructive' : 'secondary'}>
                {dueInfo.changeType === 'increase' ? '+' : '-'}{currencySymbol}{Math.abs(dueInfo.changeAmount).toFixed(2)}
              </Badge>
            )}
          </div>
          ...
          {dueInfo.changeType !== 'none' && (
            <div className="grid grid-cols-2 gap-4 mt-3 p-3 bg-white/50 rounded-md text-xs">
              <div>
                ...
                <p className="font-medium">{currencySymbol}{dueInfo.oldTotal.toFixed(2)}</p>
              </div>
              <div>
                <span className="text-muted-foreground">New Total:</span>
                <p className="font-medium">{currencySymbol}{dueInfo.newTotal.toFixed(2)}</p>
              </div>
            </div>
          )}
