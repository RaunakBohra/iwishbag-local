...
export const CustomerMetrics = ({ customers, customerAnalytics, isLoading }: CustomerMetricsProps) => {
  ...
  const totalQuotes = customerAnalytics?.reduce((sum, analytics) => sum + analytics.quoteCount, 0) || 0;
  const avgOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0;
  
  // Calculate recent activity
  const recentlyActive = customerAnalytics?.filter(analytics => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      trend: totalCustomers > 0 ? `${((activeCustomers / totalCustomers) * 100).toFixed(1)}% of total` : '0%',
      icon: UserCheck,
      color: 'green',
    ...
    },
    {
      ...
      trend: totalCustomers > 0 ? `${((vipCustomers / totalCustomers) * 100).toFixed(1)}% of total` : '0%',
      icon: Star,
      color: 'yellow',
    ...
    },
    {
      ...
      trend: totalCustomers > 0 ? `${formatCurrency(totalRevenue / totalCustomers)} per customer` : '$0',
      icon: DollarSign,
      color: 'emerald',
    ...
    },
    {
      ...
      trend: totalCustomers > 0 ? `${((recentlyActive / totalCustomers) * 100).toFixed(1)}% of total` : '0%',
      icon: Activity,
      color: 'pink',
            ...
            <Card key={index} className="border-gray-200 shadow-sm hover:shadow-md transition-shadow">
              ...
              <CardContent className="pt-0">
                <div className="flex items-baseline justify-between">
                  <Body className="text-2xl font-bold text-gray-900">
                    {metric.value}
                  </Body>
                  {metric.trendUp && (
