        ...
        <div className="space-y-6">
            ...
            {analytics && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card className={analytics.pendingProofs > 0 ? 'bg-blue-50 border-blue-200' : 'bg-gray-50'}>
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className={`text-2xl font-bold ${analytics.pendingProofs > 0 ? 'text-blue-600' : 'text-gray-600'}`}>
                                        {analytics.pendingProofs}
                                    </p>
                                    <p className="text-sm text-muted-foreground">Pending Proofs</p>
                                </div>
                                <Receipt className={`h-5 w-5 ${analytics.pendingProofs > 0 ? 'text-blue-500' : 'text-gray-400'}`} />
                            </div>
                    ...
                    <Card className={analytics.unpaidOrders > 0 ? 'bg-red-50 border-red-200' : 'bg-gray-50'}>
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className={`text-2xl font-bold ${analytics.unpaidOrders > 0 ? 'text-red-600' : 'text-gray-600'}`}>
                                        {analytics.unpaidOrders}
                                    </p>
                                    <p className="text-sm text-muted-foreground">Unpaid Orders</p>
                                </div>
                                <AlertCircle className={`h-5 w-5 ${analytics.unpaidOrders > 0 ? 'text-red-500' : 'text-gray-400'}`} />
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-2xl font-bold text-orange-600">${analytics.totalOutstanding}</p>
