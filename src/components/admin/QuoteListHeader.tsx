...
interface QuoteListHeaderProps {
  ...
  onBulkAction: (
    action: 'approved' | 'cancelled' | 'confirm_payment' | 'export' | 'priority',
  ) => void;
...
const actions = ['approved', 'cancelled', 'confirm_payment', 'export', 'priority'];
        ...
        <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
          {actions.map((action) => (
            <Button
              ...
              onClick={() =>
                onBulkAction(
                  action as 'approved' | 'cancelled' | 'confirm_payment' | 'export' | 'priority',
                )
              }
              disabled={
                isProcessing || isActionLoading(action, isUpdatingStatus, activeStatusUpdate)
              }
              className={[
