...
import PerItemTaxCalculator, { ItemTaxBreakdown, QuoteItem, TaxCalculationContext } from '@/services/PerItemTaxCalculator';
  ...
  const calculateAllItemTaxes = async () => {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const minValuationContext = { ...context, valuation_method_preference: 'minimum_valuation' };
        ...
        if (currentBreakdown) {
          ...
          newValuationData[item.id] = {
            ...
            method_comparison: {
              ...
              difference_percent: actualPriceTax > 0 ? 
                Math.abs((actualPriceTax - minValuationTax) / actualPriceTax * 100) : 0
            }
          };
        }
      }
    ...
    return (
      <div className={`space-y-3 ${className}`}>
        ...
        <div className="grid grid-cols-1 gap-2">
          {items.filter(item => item.hsn_code).map((item) => {
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {valuationData && (
                    <div className="text-xs text-gray-600">
                      Tax: {formatCurrency(valuationData.tax_preview.total_tax, valuationData.currency)}
                    </div>
                  )}
                </div>
              </div>
                  ...
                  {valuationData && (
                    <div className="mt-2 flex items-center justify-between">
                      <Badge variant="outline" className="text-xs">
                        {valuationMethods.find(m => m.id === valuationData.current_method)?.name}
                      </Badge>
                      <div className="text-xs text-gray-600">
                        Tax: {formatCurrency(valuationData.tax_preview.total_tax, valuationData.currency)}
                      </div>
                    ...
                    {valuationMethods.map((method) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => !isLoading && !isCalculating && handleValuationMethodChange(selectedItemId, method.id)}
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                              <div className={`p-1 rounded ${isSelected ? 'bg-primary text-white' : 'bg-gray-100'}`}>
                                {method.icon}
                              </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex space-x-2">
                                <Input
                                  ...
                                  onChange={(e) => handleOverrideAmountChange(selectedItemId, parseFloat(e.target.value) || 0)}
                                ...
                                <Button
                                  ...
                                  disabled={!overrideAmounts[selectedItemId] || overrideAmounts[selectedItemId] <= 0}
                                >
                                  Save
                                </Button>
                              </div>
                          ...
                          {valuationData && isSelected && (
                            <div className="mt-3 pt-3 border-t">
                              <div className="text-sm font-medium text-gray-700">
                                Current Tax: {formatCurrency(valuationData.tax_preview.total_tax, valuationData.currency)}
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="space-y-4">
                          ...
                          <Card className="p-4">
                            ...
                            <div className="space-y-1">
                              <div className="text-2xl font-bold text-blue-600">
                                {formatCurrency(itemValuationData[selectedItemId].method_comparison.actual_price_tax, itemValuationData[selectedItemId].currency)}
                              </div>
                              <div className="text-sm text-gray-600">
                                Based on item price: {formatCurrency(itemValuationData[selectedItemId].actual_price, itemValuationData[selectedItemId].currency)}
                              </div>
                            </div>
                          </Card>
                          ...
                          <Card className="p-4">
                            ...
                            <div className="space-y-1">
                              <div className="text-2xl font-bold text-green-600">
                                {formatCurrency(itemValuationData[selectedItemId].method_comparison.minimum_valuation_tax, itemValuationData[selectedItemId].currency)}
                              </div>
                              <div className="text-sm text-gray-600">
                                Based on minimum: {itemValuationData[selectedItemId].minimum_valuation ? 
                                  formatCurrency(itemValuationData[selectedItemId].minimum_valuation!, itemValuationData[selectedItemId].currency) : 
                                  'Not available'
                                }
                              </div>
                            </div>
                        ...
                        <Card className="p-4">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Badge variant={itemValuationData[selectedItemId].method_comparison.difference_percent > 10 ? "destructive" : "secondary"}>
                              {itemValuationData[selectedItemId].method_comparison.difference_percent.toFixed(1)}% difference
                            </Badge>
                          </div>
                          <div className="flex items-center space-x-2">
                            <div className="text-lg font-semibold">
                              {formatCurrency(itemValuationData[selectedItemId].method_comparison.difference_amount, itemValuationData[selectedItemId].currency)}
                            </div>
                            <ArrowRight className="h-4 w-4 text-gray-400" />
                            <div className="text-sm text-gray-600">
                              {itemValuationData[selectedItemId].method_comparison.actual_price_tax > itemValuationData[selectedItemId].method_comparison.minimum_valuation_tax
                        ...
                        <Card className="p-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-orange-600">
                                {formatCurrency(itemValuationData[selectedItemId].tax_preview.customs_amount, itemValuationData[selectedItemId].currency)}
                              </div>
                              <div className="text-sm text-gray-600">Customs Duty</div>
                            </div>
                            <div className="text-center">
                              <div className="text-2xl font-bold text-blue-600">
                                {formatCurrency(itemValuationData[selectedItemId].tax_preview.local_tax_amount, itemValuationData[selectedItemId].currency)}
                              </div>
                              <div className="text-sm text-gray-600">Local Tax (GST/VAT)</div>
                            </div>
                            <div className="text-center">
                              <div className="text-2xl font-bold text-purple-600">
                                {formatCurrency(itemValuationData[selectedItemId].tax_preview.total_tax, itemValuationData[selectedItemId].currency)}
                              </div>
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <Badge variant="outline">
                              {valuationMethods.find(m => m.id === itemValuationData[selectedItemId].current_method)?.name}
                            </Badge>
                          </div>
                        </Card>
                      </div>
