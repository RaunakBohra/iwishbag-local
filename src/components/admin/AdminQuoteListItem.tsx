...
import { Mail, Calendar, User, Package, DollarSign, Phone, MapPin } from 'lucide-react';
...
export const AdminQuoteListItem = ({ quote, isSelected, onSelect }: AdminQuoteListItemProps) => {
  ...
  // Item summary (unused in current display)
  ...
  useEffect(() => {
    ...
    async function fetchRouteCountries() {
      ...
      const result = await getQuoteRouteCountries(
        quote,
        shippingAddress,
        allCountries,
        fetchRouteById,
                    ...
                    <span className="flex items-center gap-1 flex-shrink-0 w-24">
                      ...
                      >
                        ...
                        <svg
                          ...
                          className="lucide lucide-external-link h-3 w-3 ml-0.5"
                        >
        ...
        <div className="space-y-2">
              ...
              <>
                ...
                {quote.quote_items && quote.quote_items.length > 1 && (
                  <span className="text-muted-foreground ml-1">
                    +{quote.quote_items.length - 1} more
                  </span>
                )}
              </>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-bold">
              {currencyDisplays[0]?.amount} {currencyDisplays[0]?.currency}
            </span>
            <span className="text-muted-foreground">
              ({quote.quote_items?.length || 0} item
              {(quote.quote_items?.length || 0) !== 1 ? 's' : ''})
            </span>
          </div>
          {routeCountries && (
            <div className="flex items-center gap-2 text-sm">
