...
export const EnhancedDeliveryTimeline: React.FC<EnhancedDeliveryTimelineProps> = ({
  ...
  const getPhaseStatus = (phase: DeliveryTimeline) => {
    if (currentDate < phase.start_date) {
      return { status: 'pending', color: 'text-gray-400', bgColor: 'bg-gray-100' };
    } else if (currentDate >= phase.start_date && currentDate <= phase.end_date) {
      return { status: 'in_progress', color: 'text-blue-600', bgColor: 'bg-blue-100' };
    } else {
            ...
            return (
              <div key={phase.phase} className="relative">
                ...
                <div className="flex items-start gap-4">
                  {/* Phase Icon */}
                  <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${phaseStatus.bgColor}`}>
                    <div className={phaseStatus.color}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>{phase.duration_days} day{phase.duration_days > 1 ? 's' : ''}</span>
        ...
        <div className="mt-6 pt-4 border-t">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-500">
              {timeline.filter(p => getPhaseStatus(p).status === 'completed').length} of {timeline.length} phases
            </span>
          </div>
          <Progress 
            value={(timeline.filter(p => getPhaseStatus(p).status === 'completed').length / timeline.length) * 100} 
            className="h-3" 
          />
        </div>
      </CardContent>
