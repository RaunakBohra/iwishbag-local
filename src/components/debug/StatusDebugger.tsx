...
export const StatusDebugger = () => {
  const { quoteStatuses, orderStatuses, getStatusesForQuotesList, getStatusesForOrdersList, findStatusForPaymentMethod } = useStatusManagement();
  ...
  useEffect(() => {
    ...
    quoteStatuses?.forEach(status => {
      ...
      if (status.name === 'payment_pending') {
        console.log(`payment_pending found in quote statuses: showsInQuotesList=${status.showsInQuotesList}`);
      }
    });
    ...
    orderStatuses?.forEach(status => {
      ...
      if (status.name === 'payment_pending' || status.name === 'payment pending') {
        console.log(`Payment status found: "${status.name}" showsInOrdersList=${status.showsInOrdersList}`);
      }
    });
    ...
    console.log('payment_pending allowed in quotes?', quotesListStatuses.includes('payment_pending'));
    console.log('payment_pending allowed in orders?', ordersListStatuses.includes('payment_pending'));
    console.log('"payment pending" (space) allowed in orders?', ordersListStatuses.includes('payment pending'));
    ...
    if (paymentPendingQuotes) {
      ...
      console.log(`Found ${paymentPendingQuotes.length} quotes with payment_pending status:`, paymentPendingQuotes);
    }
  ...
  }, [quoteStatuses, orderStatuses, paymentPendingQuotes, allQuotes]);
