...
const DuplicateComponentsPreview = () => {
      ...
      items: [
        {
          ...
          description:
            'Complete currency display with tooltips, badges, exchange rate info, stories file included',
          features: [
            'Tooltips',
            'Exchange rates',
            'Multiple currencies',
    ...
    tables: {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            'Enhanced table with some advanced features but not as complete as WorldClass',
  ...
  const totalLines = Object.values(duplicateGroups).reduce(
    (acc, group) => acc + group.items.reduce((sum, item) => sum + item.lines, 0),
    0,
  );

  const getStatusColor = (status: string) => {
    if (status.includes('Production') || status.includes('Enhanced') || status.includes('Fixed'))
      return 'bg-green-100 text-green-800';
    if (status.includes('Basic') || status.includes('Legacy'))
      return 'bg-yellow-100 text-yellow-800';
    if (status.includes('Duplicate') || status.includes('Original'))
                ...
                <Card key={item.id} className="hover:shadow-md transition-shadow">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="border-t pt-3">
                      ...
                      <p
                        className={`text-sm font-medium ${getRecommendationColor(item.recommendation)}`}
                      >
                        {item.recommendation}
                      </p>
                    </div>
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="p-6">
          ...
          <div className="space-y-2 text-sm text-blue-800">
            <p>
              1. <strong>Browse each category</strong> using the tabs above
            </p>
            <p>
              2. <strong>Review each component</strong> - check the features, file paths, and
              recommendations
            </p>
            <p>
              3. <strong>Tell me which components to remove</strong> by their numbers (e.g., "Remove
              #2, #5, #6, #8")
            </p>
            <p>
              4. <strong>I'll handle the cleanup</strong> - migrate usage, remove files, update
              imports
            </p>
          </div>
