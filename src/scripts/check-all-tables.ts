...
async function checkAllTables() {
  ...
  if (payments && payments.length > 0) {
    payments.forEach((p) => {
      console.log(
        `- ${p.payment_method}: ${p.amount} ${p.currency} (${p.status}) at ${new Date(p.created_at).toLocaleTimeString()}`,
      );
    });
  } else {
  ...
  if (logs && logs.length > 0) {
    logs.forEach((l) => {
      console.log(
        `- ${l.webhook_type}: ${l.status || 'processed'} at ${new Date(l.created_at).toLocaleTimeString()}`,
      );
  ...
  if (quotes && quotes.length > 0) {
    quotes.forEach((q) => {
      console.log(
        `- Quote ${q.id.substring(0, 8)}: ${q.status} (payment: ${q.payment_status}) via ${q.payment_method} at ${new Date(q.updated_at).toLocaleTimeString()}`,
      );
    });
  } else {
    console.log('- None found');
  ...
  if (ledger && ledger.length > 0) {
    ledger.forEach((l) => {
      console.log(
        `- ${l.payment_type}: ${l.amount} ${l.currency} via ${l.gateway_code} at ${new Date(l.created_at).toLocaleTimeString()}`,
      );
    });
  } else {
    console.log('- None found');
