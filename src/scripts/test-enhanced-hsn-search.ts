...
export async function testEnhancedHSNSearch() {
  ...
  try {
    ...
    console.log(`✅ Learning initialized: ${learningResult.learned} new mappings, ${learningResult.updated} updated`);
    ...
    const contextualResults = await enhancedHSNSearchService.getContextualSuggestions('Apple Watch', 5);
    ...
    const brandResults = await enhancedHSNSearchService.detectHSNFromProductName('Sony PlayStation 5');
