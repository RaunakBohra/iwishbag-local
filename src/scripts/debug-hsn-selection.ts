...
async function debugHSNSelection() {
  ...
  try {
    ...
    page.on('request', (request) => {
      ...
      if (method === 'POST' || url.includes('submit') || request.headers()['content-type']?.includes('form')) {
        const activity = {
          method,
          url,
    ...
    await page.addInitScript(() => {
      document.addEventListener('click', (e) => {
        ...
        if (target.textContent?.includes('HSN') || 
    ...
    try {
      ...
      if (await dialog.isVisible()) {
        ...
        if (hsnResultCard) {
          ...
          const hsnCode = await hsnResultCard.locator('text=/\\d{8}/').textContent().catch(() => 'Unknown');
          ...
          console.log(`   ${beforeSelectUrl === afterSelectUrl ? '✅ No navigation occurred' : '⚠️ Page navigated!'}`);
          ...
          const appliedHSN = await page.locator(`text=${hsnCode}`).first().isVisible().catch(() => false);
