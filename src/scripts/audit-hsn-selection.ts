  ...
  try {
    ...
    if (await hsnSearchButton.isVisible()) {
      ...
      if (await dialog.isVisible()) {
        ...
        const hsnResults = await page.locator(
          '[data-testid="hsn-result-item"], .hsn-result-item, [class*="hsn-result"]',
        );
        ...
        if (resultCount > 0) {
          ...
          const hsnCodeElement = await page
            .locator('.hsn-code, [data-testid="hsn-code"], text=/\\d{8}/')
            .first();
          ...
          const sidebarHSN = await page.locator(
            '[data-testid="sidebar-hsn"], .sidebar-hsn, aside >> text=/HSN/',
          );
