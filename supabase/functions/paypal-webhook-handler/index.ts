...
const PAYPAL_BASE_URL =
  Deno.env.get('PAYPAL_ENVIRONMENT') === 'sandbox'
    ...
    switch (eventType) {
      ...
      case 'CHECKOUT.ORDER.COMPLETED':
        ...
        const amount = parseFloat(
          capture?.amount?.value || resource.amount?.value || paymentTx.amount,
        );
        const currency =
          capture?.amount?.currency_code || resource.amount?.currency_code || paymentTx.currency;
