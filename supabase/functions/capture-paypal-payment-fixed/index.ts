...
serve(async (req)=>{
  ...
  try {
    ...
    let { data: updatedRows, error: updateError } = await supabaseAdmin.from('payment_transactions').update({
      status: 'completed',
      paypal_capture_id: capture?.id,
      gateway_response: captureData,
      updated_at: new Date().toISOString()
    }).eq('paypal_order_id', orderId).select();
    ...
    if (!updatedRows || updatedRows.length === 0) {
      ...
      const result = await supabaseAdmin.from('payment_transactions').update({
        status: 'completed',
        paypal_capture_id: capture?.id,
        paypal_order_id: orderId,
        gateway_response: captureData,
        updated_at: new Date().toISOString()
      }).eq('gateway_response->>paypal_order_id', orderId).select();
    ...
    // Continue execution - we still want to update quotes if possible
      ...
      const { data: existingTxn } = await supabaseAdmin.from('payment_transactions').select('id, paypal_order_id, status, gateway_response').or(`paypal_order_id.eq.${orderId},gateway_response->>paypal_order_id.eq.${orderId}`).single();
    ...
    if (quoteIds.length > 0) {
      ...
      const { data: updatedQuotes, error: quoteError } = await supabaseAdmin.from('quotes').update({
        status: 'paid',
        payment_status: 'paid',
        paid_at: new Date().toISOString()
      }).in('id', quoteIds).select();
      if (quoteError) {
