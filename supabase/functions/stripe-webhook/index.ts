...
serve(async (req) => {
  ...
  return await withEdgeMonitoring('stripe-webhook', async (logger, paymentMonitoring) => {
    try {
    ...
    const supabaseAdmin = createClient(
      ...
      const { data: stripeGateway, error: configError } = await paymentMonitoring.monitorGatewayCall(
        'fetch_stripe_config',
        'stripe',
        async () => {
    ...
    const webhookSecret = config.webhook_secret
      ...
      try {
        ...
        logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Webhook signature verified successfully', {
          metadata: {
            eventId: event.id,
      ...
      logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, `Processing Stripe webhook: ${event.type}`, {
        metadata: {
          webhookId,
          eventId: event.id,
          paymentId,
    ...
    let processingError: string | undefined
      ...
      if (processingSuccess) {
        logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Webhook processing completed successfully', {
          metadata: {
            webhookId,
            eventType: event.type,
            eventId: event.id
        ...
        return createSuccessResponse(responseData, 200, logger, { webhookId, eventType: event.type });
      } else {
        logger.error(
          EdgeLogCategory.WEBHOOK_PROCESSING,
