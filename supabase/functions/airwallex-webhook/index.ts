...
serve(async (req) => {
  ...
  return await withEdgeMonitoring('airwallex-webhook', async (logger, paymentMonitoring) => {
    try {
      ...
      const { data: airwallexGateway, error: configError } = await paymentMonitoring.monitorGatewayCall(
      ...
      try {
        ...
        isValidSignature = await verifyAirwallexWebhookSignature(
          signature,
          webhookSecret,
          body
        );
        ...
        if (isValidSignature) {
          logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Airwallex webhook signature verified successfully', {
            metadata: {
        ...
        return createErrorResponse(
          new Error(`Webhook signature verification failed: ${err instanceof Error ? err.message : 'Unknown error'}`),
          400,
          logger
        );
      }
      ...
      const orderId = event.data.object?.metadata?.orderId || event.data.object?.metadata?.order_id;
      ...
      logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, `Processing Airwallex webhook: ${event.name}`, {
        metadata: {
          webhookId,
          eventId: event.id,
          accountId: event.account_id,
      ...
      switch (event.name) {
        case 'payment_intent.succeeded': {
          ...
          const result = await paymentMonitoring.monitorGatewayCall(
            ...
            'airwallex',
            async () => await processPaymentIntentSucceeded(supabaseAdmin, event.data.object as any),
            paymentId
          );
          ...
          if (result.success) {
            logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Payment intent succeeded processed', {
              metadata: {
                webhookId,
                eventId: event.id,
        ...
        case 'payment_intent.failed': {
          ...
          const result = await paymentMonitoring.monitorGatewayCall(
            ...
            'airwallex',
            async () => await processPaymentIntentFailed(supabaseAdmin, event.data.object as any, 'failed'),
            paymentId
          );
        ...
        case 'payment_intent.cancelled': {
          ...
          const result = await paymentMonitoring.monitorGatewayCall(
            ...
            'airwallex',
            async () => await processPaymentIntentFailed(supabaseAdmin, event.data.object as any, 'cancelled'),
            paymentId
          );
        ...
        case 'payment_attempt.settled': {
          ...
          if (result.success) {
            logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Payment attempt settled processed as succeeded', {
              metadata: {
                webhookId,
                eventId: event.id,
        ...
        default:
          logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, `Unhandled Airwallex event type: ${event.name}`, {
            metadata: {
              webhookId,
              eventType: event.name,
              eventId: event.id
      ...
      if (processingSuccess) {
        logger.info(EdgeLogCategory.WEBHOOK_PROCESSING, 'Airwallex webhook processing completed successfully', {
          metadata: {
            webhookId,
            eventType: event.name,
            eventId: event.id
        ...
        return createSuccessResponse(responseData, 200, logger, { webhookId, eventType: event.name });
      } else {
        logger.error(
          EdgeLogCategory.WEBHOOK_PROCESSING,
