...
serve(async (req)=>{
  ...
  try {
    ...
    const { invoice_id, subject, note, send_to_recipient = true, send_to_invoicer = false } = sendRequest;
    ...
    const { data: invoice, error: invoiceError } = await supabaseAdmin.from('paypal_invoices').select(`
        *,
        quotes (
          display_id,
          product_name,
    ...
    const sendInvoiceRequest = {
      ...
      note: note || `Please find attached your invoice for quote ${invoice.quotes.display_id}. Payment is due by ${invoice.payment_due_date}.`,
    ...
    const sendResponse = await fetch(`${paypalConfig.base_url}/v2/invoicing/invoices/${invoice.paypal_invoice_id}/send`, {
      method: 'POST',
      headers: {
    ...
    const { data: updatedInvoice, error: updateError } = await supabaseAdmin.from('paypal_invoices').update({
      status: 'sent',
