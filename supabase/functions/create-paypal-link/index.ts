...
serve(async (req)=>{
  ...
  try {
    ...
    const { title, description, amount, currency = 'USD', quote_id, user_id, expires_in_hours = 48, max_uses = 1, allow_partial_payment = false, minimum_payment_amount, payment_note, custom_redirect_url, is_public = true, metadata = {} } = linkRequest;
    ...
    if (quote_id && quote_id.trim() !== '') {
      const { data: quote, error: quoteError } = await supabaseAdmin.from('quotes').select('id, status').eq('id', quote_id).single();
      if (quoteError || !quote) {
    ...
    if (user_id && user_id.trim() !== '') {
      const { data: profile, error: profileError } = await supabaseAdmin.from('profiles').select('id').eq('id', user_id).single();
    ...
    const { data: linkCode, error: codeError } = await supabaseAdmin.rpc('generate_payment_link_code');
    ...
    const expiresAt = expires_in_hours > 0 ? new Date(Date.now() + expires_in_hours * 60 * 60 * 1000).toISOString() : null;
    ...
    const insertData = {
      ...
      minimum_payment_amount: minimum_payment_amount && minimum_payment_amount > 0 ? minimum_payment_amount : null,
      payment_note: payment_note && payment_note.trim() !== '' ? payment_note : null,
      custom_redirect_url: custom_redirect_url && custom_redirect_url.trim() !== '' ? custom_redirect_url : null,
      is_public: is_public,
    ...
    const { data: paymentLink, error: linkError } = await supabaseAdmin.from('paypal_payment_links').insert(insertData).select().single();
    if (linkError) {
