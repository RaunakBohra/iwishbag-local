...
export function isCurrencySupportedByAirwallex(currency: string): boolean {
  ...
  const supportedCurrencies = [
    'USD', 'EUR', 'GBP', 'CAD', 'AUD', 'JPY', 'SGD', 'HKD', 'CNY',
    'NZD', 'THB', 'KRW', 'PHP', 'IDR', 'MYR', 'VND', 'INR', 'CHF',
    'SEK', 'NOK', 'DKK', 'PLN', 'CZK', 'HUF', 'RON', 'BGN', 'HRK',
    'AED', 'SAR', 'QAR', 'KWD', 'BHD', 'OMR', 'JOD', 'ILS', 'TRY',
    'ZAR', 'NGN', 'GHS', 'KES', 'EGP', 'MAD', 'TND'
  ];
 ...
 */
async function createPaymentWithoutAuth(params: CreateAirwallexPaymentParams): Promise<AirwallexPaymentResponse> {
  const {
    apiKey,
    clientId,
    testMode,
 ...
 */
async function getAirwallexAccessToken(apiKey: string, clientId: string, testMode: boolean): Promise<string | null> {
  try {
    console.log('🔐 Getting Airwallex access token...', {
      testMode,
      authUrl: testMode ? 'https://api-demo.airwallex.com/api/v1/authentication/login' : 'https://api.airwallex.com/api/v1/authentication/login',
      hasApiKey: !!apiKey,
      hasClientId: !!clientId,
