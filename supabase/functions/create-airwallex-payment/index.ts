...
serve(async (req)=>{
  ...
  try {
    ...
    const { quote_ids: quoteIds, success_url, cancel_url, metadata, customer_info: customerInfo } = paymentRequest;
    ...
    const { data: quotes, error: quotesError } = await supabaseAdmin.from('quotes').select('*').in('id', quoteIds);
    ...
    const { data: airwallexGateway, error: gatewayError } = await supabaseAdmin.from('payment_gateways').select('config, test_mode').eq('code', 'airwallex').single();
    ...
    const apiBaseUrl = config.api_base_url || (testMode ? 'https://api-demo.airwallex.com' : 'https://api.airwallex.com');
    ...
    const { data: transaction, error: transactionError } = await supabaseAdmin.from('payment_transactions').insert({
      user_id: userId,
      quote_ids: quoteIds,
      amount: totalAmount,
      currency: totalCurrency,
