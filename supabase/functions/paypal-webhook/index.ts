      ...
      switch (event.event_type) {
        ...
        case 'CHECKOUT.ORDER.APPROVED':
          ...
          if (event.resource?.id) {
            ...
            const { data: updated } = await supabaseAdmin
              ...
              .eq('paypal_order_id', event.resource.id)
              .select();
        ...
        case 'PAYMENT.CAPTURE.COMPLETED':
          ...
          if (event.resource?.id && event.resource?.purchase_units?.[0]) {
            ...
            const { data: updated, error: txnUpdateError } = await supabaseAdmin
              .from('payment_transactions')
              .update({
                status: 'completed',
                paypal_order_id: event.resource.id,
            ...
            if (!updated || updated.length === 0) {
              ...
              if (jsonbUpdateError) {
                console.error(
                  'Error updating payment transaction (JSONB search):',
                  jsonbUpdateError,
                );
              }
            }
...
async function processInvoicePaidEvent(supabaseAdmin, webhookEvent) {
  ...
  const paymentAmount = webhookEvent.resource?.amount
...
async function processSubscriptionCreatedEvent(supabaseAdmin, webhookEvent) {
  ...
  const { data: subscription1, error: findError } = await supabaseAdmin
    .from('paypal_subscriptions')
    .select('id, status, webhook_events_count')
...
async function processSubscriptionActivatedEvent(supabaseAdmin, webhookEvent) {
  console.log(
    'Processing subscription activated event for subscription:',
...
async function processSubscriptionUpdatedEvent(supabaseAdmin, webhookEvent) {
  ...
  const { data: subscription1, error: findError } = await supabaseAdmin
    .from('paypal_subscriptions')
    .select('id, webhook_events_count')
    .eq('paypal_subscription_id', paypalSubscriptionId)
    .single();
  if (findError || !subscription1) {
    return {
...
async function processSubscriptionCancelledEvent(supabaseAdmin, webhookEvent) {
  console.log(
    'Processing subscription cancelled event for subscription:',
    webhookEvent.resource?.id,
  );
  const paypalSubscriptionId = webhookEvent.resource?.id;
  if (!paypalSubscriptionId) {
...
async function processSubscriptionSuspendedEvent(supabaseAdmin, webhookEvent) {
  console.log(
    'Processing subscription suspended event for subscription:',
    webhookEvent.resource?.id,
  );
  const paypalSubscriptionId = webhookEvent.resource?.id;
  if (!paypalSubscriptionId) {
...
async function processSubscriptionPaymentFailedEvent(supabaseAdmin, webhookEvent) {
  console.log(
    'Processing subscription payment failed event for subscription:',
    webhookEvent.resource?.id,
  );
  const paypalSubscriptionId = webhookEvent.resource?.id;
  if (!paypalSubscriptionId) {
  ...
  const { data: subscription1, error: findError } = await supabaseAdmin
    .from('paypal_subscriptions')
    .select('id, failed_payments, webhook_events_count')
    .eq('paypal_subscription_id', paypalSubscriptionId)
    .single();
  if (findError || !subscription1) {
    return {
...
async function processSubscriptionPaymentCompletedEvent(supabaseAdmin, webhookEvent) {
  console.log(
    'Processing subscription payment completed event for payment:',
    webhookEvent.resource?.id,
  );
  ...
  const { data: subscription1, error: findError } = await supabaseAdmin
    .from('paypal_subscriptions')
    .select('id, user_id, completed_cycles, total_paid, webhook_events_count')
