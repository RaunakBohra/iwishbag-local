...
serve(async (req)=>{
  ...
  try {
    ...
    const { plan_id, return_url = 'https://whyteclub.com/dashboard?subscription=success', cancel_url = 'https://whyteclub.com/dashboard?subscription=cancelled', custom_id, subscriber_info } = subscriptionRequest;
    ...
    const { data: plan, error: planError } = await supabaseAdmin.from('paypal_subscription_plans').select('*').eq('id', plan_id).single();
    if (planError || !plan) {
    ...
    const { data: existingSubscription } = await supabaseAdmin.from('paypal_subscriptions').select('id, status, paypal_subscription_id').eq('user_id', user.id).eq('plan_id', plan_id).in('status', [
      'approval_pending',
      'approved',
      'active'
    ]).single();
    ...
    const { data: profile } = await supabaseAdmin.from('profiles').select('full_name, email, phone, country').eq('id', user.id).single();
    ...
    if (isTestMode) {
      ...
      const subscriptionResponse = await fetch(`${paypalConfig.base_url}/v1/billing/subscriptions`, {
        method: 'POST',
        headers: {
      ...
      console.log('PayPal Subscription API Response Data:', JSON.stringify(subscriptionData, null, 2));
    ...
    const { data: subscription, error: subscriptionDbError } = await supabaseAdmin.from('paypal_subscriptions').insert({
      user_id: user.id,
      plan_id: plan_id,
      ...
      trial_end_date: plan.trial_days > 0 ? new Date(Date.now() + plan.trial_days * 24 * 60 * 60 * 1000).toISOString() : null,
    ...
    return new Response(JSON.stringify({
      ...
      paypal_response: paypalSubscription,
      test_mode_info: isTestMode ? 'This is a test subscription. In production, replace the TEMP- plan IDs with real PayPal plan IDs.' : undefined
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
