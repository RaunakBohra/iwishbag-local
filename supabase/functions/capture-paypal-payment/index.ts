...
serve(async (req) => {
  ...
  try {
    ...
    const clientId =
      config.client_id || (testMode ? config.client_id_sandbox : config.client_id_live);
    const clientSecret =
      config.client_secret || (testMode ? config.client_secret_sandbox : config.client_secret_live);
