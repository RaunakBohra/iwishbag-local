...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: quote, error } = await supabaseClient.from('automatic_quotes').insert({
    ...
    return new Response(JSON.stringify({
      success: true,
      quote_id: quote.id,
      message: body.review_requested ? 'Quote submitted for expert review. You will receive an email within 24 hours.' : 'Quote submitted successfully. You can accept it anytime within 24 hours.'
    }), {
      status: 200,
      headers: {
