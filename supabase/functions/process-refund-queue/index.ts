...
serve(async (req) => {
  ...
  try {
    ...
    for (const queueItem of processResult || []) {
      console.log(
        `🔄 Processing queued refund ${queueItem.queue_id} with status: ${queueItem.status}`,
      );
    ...
    const { data: stats } = await supabaseAdmin.rpc('get_refund_retry_stats', {
      p_gateway_code: gatewayCode,
      p_days_back: 7,
    });
