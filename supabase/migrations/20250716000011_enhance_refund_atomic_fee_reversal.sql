-- Drop existing function to modify for fee handling
DROP FUNCTION IF EXISTS process_refund_atomic(UUID, DECIMAL(10,2), JSONB, JSONB, UUID);

-- Create enhanced version with fee reversal handling
CREATE OR REPLACE FUNCTION process_refund_atomic(
  ...
  fee_reversal_entry_id UUID,
  error_message TEXT
) AS $$
DECLARE
  v_refund_id UUID;
  v_ledger_entry_id UUID;
  v_fee_reversal_entry_id UUID;
  ...
  v_original_fee_amount DECIMAL(10,2);
  v_refund_fee_amount DECIMAL(10,2);
  v_refund_percentage DECIMAL(5,3);
BEGIN
  ...
  BEGIN
    ...
    -- Get the original payment transaction with fee information
    ...
    v_original_fee_amount := COALESCE(v_original_transaction.gateway_fee_amount, 0);
    ...
    IF v_original_fee_amount > 0 AND v_original_transaction.amount > 0 THEN
      -- Calculate refund percentage
      v_refund_percentage := (p_refund_amount / v_original_transaction.amount)::DECIMAL(5,3);
      
      -- Calculate proportional fee refund
      -- Note: Some gateways refund fees, others don't - this assumes proportional refund
      v_refund_fee_amount := ROUND((v_original_fee_amount * v_refund_percentage)::DECIMAL(10,2), 2);
    ELSE
      v_refund_fee_amount := 0;
    END IF;
    ...
    -- Create fee reversal ledger entry if applicable
    IF v_refund_fee_amount > 0 THEN
      INSERT INTO payment_ledger (
        quote_id,
        payment_transaction_id,
        payment_type,
        ...
        p_quote_id,
        v_payment_transaction_id,
        'gateway_fee_reversal',
        v_refund_fee_amount, -- Positive for fee reversal (credit back)
        COALESCE(v_currency, v_original_transaction.currency),
        'payu',
        'payu',
        v_gateway_transaction_id,
        v_gateway_refund_id || '_FEE_REVERSAL',
        'processing',
        NOW(),
        v_refund_fee_amount,
        -v_original_fee_amount,
        -v_original_fee_amount + v_refund_fee_amount,
        'Gateway fee reversal for refund',
        p_processed_by,
        jsonb_build_object(
          'fee_reversal', true,
          'original_fee_amount', v_original_fee_amount,
          'refund_fee_amount', v_refund_fee_amount,
          'refund_percentage', v_refund_percentage,
          'parent_refund_id', v_refund_id,
          'processed_at', NOW()
        ),
        NOW(),
        NOW()
      ) RETURNING id INTO v_fee_reversal_entry_id;
    END IF;
...
-- Add comment
COMMENT ON FUNCTION process_refund_atomic IS 'Enhanced atomic refund processing with proportional fee reversal handling. Creates separate ledger entries for fee reversals to maintain complete financial audit trail.';