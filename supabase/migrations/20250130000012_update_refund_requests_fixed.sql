...
-- =====================================================
-- PART 8: UPDATE APPLICATION FUNCTIONS
-- =====================================================

-- Update any functions that reference payment_ledger_id
-- This is a helper function to find payment transactions for refunds
CREATE OR REPLACE FUNCTION find_payment_for_refund(p_quote_id uuid, p_amount numeric)
RETURNS uuid
LANGUAGE sql
SECURITY DEFINER
STABLE
AS $$
    ...
    AND payment_type IN ('customer_payment', NULL)
    AND amount >= p_amount
...
COMMENT ON FUNCTION find_payment_for_refund IS 'Find the most recent payment transaction for a quote that can be refunded';

-- =====================================================
-- PART 10: MIGRATION STATUS
-- =====================================================

DO $$
DECLARE
    v_total_refunds integer;
    v_migrated_refunds integer;
BEGIN
    -- Count total and migrated refunds
    SELECT COUNT(*) INTO v_total_refunds FROM refund_requests WHERE payment_ledger_id IS NOT NULL;
    SELECT COUNT(*) INTO v_migrated_refunds FROM refund_requests WHERE payment_transaction_id IS NOT NULL;
    
    -- Log migration status
    RAISE NOTICE 'Refund requests migration completed:';
    RAISE NOTICE '- Total refunds with payment_ledger_id: %', v_total_refunds;
    RAISE NOTICE '- Successfully migrated: %', v_migrated_refunds;
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'TODO: After verification, drop payment_ledger_id column with:';
    RAISE NOTICE 'ALTER TABLE refund_requests DROP COLUMN payment_ledger_id;';
END $$;
