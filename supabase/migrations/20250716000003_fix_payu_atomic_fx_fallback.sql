-- Fix critical financial risk: Remove hardcoded exchange rate fallback from process_payu_payment_atomic
-- This ensures transactions fail explicitly when exchange rates are unavailable
...
BEGIN
  ...
  BEGIN
    ...
    -- CRITICAL: NO FALLBACK - Transaction must fail if exchange rate is unavailable
    IF v_exchange_rate IS NULL OR v_exchange_rate <= 0 THEN
      RETURN QUERY SELECT 
        ...
        FALSE,
        NULL::UUID,
        'CRITICAL ERROR: Exchange rate for INR is unavailable or invalid. Transaction cannot proceed without valid exchange rate.'::TEXT;
      RETURN;
    END IF;
...
-- Update comment to reflect the critical fix
COMMENT ON FUNCTION process_payu_payment_atomic IS 'Atomically processes PayU payment completion. CRITICAL: Fails explicitly if exchange rate is unavailable to prevent financial inaccuracies.';