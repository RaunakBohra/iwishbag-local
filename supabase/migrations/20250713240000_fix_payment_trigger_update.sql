-- Fix the RPC function to properly trigger the payment status update
...
BEGIN
  ...
  -- CRITICAL: Trigger the payment status update by updating amount_paid
  -- This fires the update_payment_status_on_quote trigger which recalculates everything
  UPDATE quotes
  SET amount_paid = amount_paid  -- This triggers the BEFORE UPDATE trigger
  WHERE id = force_update_payment.quote_id;
  
  RAISE NOTICE 'Triggered payment status recalculation';
