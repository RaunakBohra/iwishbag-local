-- Fix payment verification to create payment_ledger entries
-- This updates the force_update_payment function to use the new payment_ledger system

-- Drop the old function
DROP FUNCTION IF EXISTS force_update_payment(UUID, DECIMAL, TEXT);

-- Create new enhanced function that creates payment_ledger entries
...
BEGIN
  ...
  -- Get quote details
  SELECT * INTO quote_info FROM quotes WHERE id = quote_id;
  IF NOT FOUND THEN
  ...
  -- Calculate existing payment records total (USD)
  SELECT COALESCE(SUM(amount), 0) INTO existing_records_total
  FROM payment_records 
  WHERE quote_id = quote_id;
  
  -- Calculate existing payment ledger total (USD equivalent)
  ...
  ELSIF new_record_amount < 0 THEN
    ...
    DELETE FROM payment_records WHERE quote_id = quote_id;
    DELETE FROM payment_ledger WHERE quote_id = quote_id AND payment_type = 'customer_payment';
...
-- Create a trigger function to sync payment_records to payment_ledger
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        SELECT COALESCE(SUM(base_amount), 0) INTO balance_before
        FROM payment_ledger
        ...
        -- Create corresponding payment_ledger entry if it doesn't exist
        INSERT INTO payment_ledger (
            quote_id,
            payment_date,
            payment_type,
            ...
            CASE 
                ...
                ELSE 'manual'
            END,
            NEW.amount * exchange_rate, -- Convert to original currency
            quote_info.final_currency,
            exchange_rate,
        ...
        RAISE NOTICE 'Auto-synced payment_record % to payment_ledger', NEW.id;
    END IF;
...
-- Create trigger to auto-sync payment_records to payment_ledger
-- Only create if it doesn't exist to avoid errors
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'sync_payment_record_to_ledger_trigger'
    ) THEN
        CREATE TRIGGER sync_payment_record_to_ledger_trigger
            AFTER INSERT ON payment_records
            FOR EACH ROW
            EXECUTE FUNCTION sync_payment_record_to_ledger();
    END IF;
END $$;
...
-- Add comment
COMMENT ON FUNCTION force_update_payment IS 'Enhanced payment update function that creates both payment_records and payment_ledger entries for verification workflow';
COMMENT ON FUNCTION sync_payment_record_to_ledger IS 'Auto-sync payment_records to payment_ledger for backward compatibility';