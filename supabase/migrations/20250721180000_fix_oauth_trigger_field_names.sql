-- Migration: Fix OAuth trigger field names for auth.users table
-- The field is raw_user_meta_data, not user_metadata in auth.users

-- Updated function with correct field names
...
BEGIN
  ...
  ELSIF provider IS NULL THEN
    ...
    -- Only process if raw_user_meta_data exists and is not null
    IF NEW.raw_user_meta_data IS NOT NULL AND NEW.phone IS NULL THEN
      BEGIN
        extracted_phone := COALESCE(
          NEW.raw_user_meta_data->>'phone',                -- Manual signup phone
          NEW.raw_user_meta_data->>'phone_number'          -- Alternative format
        );
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          RAISE NOTICE 'Could not extract phone from raw_user_meta_data for user %: %', NEW.id, SQLERRM;
      END;
    END IF;
...
'Extracts phone numbers from OAuth provider metadata (Google only) and manual signups. Uses correct auth.users field names (raw_user_meta_data).';