...
async function testAirwallexHeaders() {
  ...
  try {
    ...
    if (authResponse.ok && authData.token) {
      ...
      const optionsResponse = await fetch('https://api-demo.airwallex.com/api/v1/pa/payment_intents', {
        method: 'OPTIONS',
        headers: {
      ...
      const minimalResponse = await fetch('https://api-demo.airwallex.com/api/v1/pa/payment_intents', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer ' + authData.token,
          'Content-Type': 'application/json'
