...
async function discoverAirwallexAPI() {
  ...
  try {
    ...
    if (authResponse.ok && authData.token) {
      ...
      const listResponse = await fetch('https://api-demo.airwallex.com/api/v1/pa/payment_intents?page_size=5', {
        method: 'GET',
        headers: {
      ...
      const sessionResponse = await fetch('https://api-demo.airwallex.com/api/v1/pa/payment_sessions', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer ' + authData.token,
          'Content-Type': 'application/json'
