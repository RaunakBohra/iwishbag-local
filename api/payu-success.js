export default function handler(req, res) {
  console.log('PayU Success Handler - Method:', req.method);
  console.log('PayU Success Handler - Body:', req.body);
  ...
  if (req.method === 'POST') {
    ...
    // For GET requests, just redirect to payment success
    const queryString = req.url.includes('?') ? req.url.split('?')[1] : '';
    res.redirect(302, `/payment-success?${queryString}`);
  }
}