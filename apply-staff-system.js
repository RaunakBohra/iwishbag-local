...
async function applyStaffSystem() {
  ...
  try {
    ...
    const migration = readFileSync('supabase/migrations/20250719600000_enhanced_staff_system.sql', 'utf8')
    ...
    const rolePermissions = [
      {
        ...
        permissions: ['quotes.read', 'quotes.update_status', 'messages.read', 'messages.create', 'customers.read', 'orders.read'],
        department: 'customer_service'
      },
      {
        role: 'quote_specialist', 
        permissions: ['quotes.read', 'quotes.create', 'quotes.update', 'quotes.calculate', 'products.read', 'shipping.read', 'customers.read'],
        department: 'quotes'
      },
      {
        role: 'accountant',
        permissions: ['payments.read', 'payments.verify', 'orders.read', 'financial_reports.read', 'payment_proof.review', 'quotes.read'],
        department: 'accounting'
      },
      {
        role: 'fulfillment',
        permissions: ['orders.read', 'orders.update_status', 'shipping.read', 'shipping.update', 'tracking.create', 'quotes.read'],
    ...
    console.log('\n📋 What you can now do:')
    console.log('• Assign staff to departments (customer_service, quotes, accounting, fulfillment)')
    ...
    console.log('\n🔧 Next Steps:')
    console.log('1. Create staff accounts with: role=customer_service, department=customer_service')
