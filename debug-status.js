...
async function debugStatusForDatabase(supabase, dbName) {
  ...
  try {
    ...
    if (!settings || settings.length === 0) {
      console.log('No status settings found in database. This means StatusConfigProvider should initialize defaults.');
    ...
    if (quoteSettings) {
      ...
      const allowedInQuotes = quoteStatuses
        .filter(s => s.showsInQuotesList)
        .map(s => s.name);
      
      console.log('Statuses allowed in quotes list:', allowedInQuotes);
      console.log('Does payment_pending appear in allowed list?', allowedInQuotes.includes('payment_pending'));
    }
    ...
    if (orderSettings) {
      ...
      const allowedInOrders = orderStatuses
        .filter(s => s.showsInOrdersList)
        .map(s => s.name);
      
      console.log('Statuses allowed in orders list:', allowedInOrders);
      console.log('Does payment_pending appear in allowed list?', allowedInOrders.includes('payment_pending'));
    }
